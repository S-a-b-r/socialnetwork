        ...
        <div class="card">
                    ...
                    @if(auth()->user()->id == $book->author->id)
                        ...
                        @else
                            <form method="post" action="{{route('books.make.link', $book)}}">
                                @csrf
                                <button type="submit" class="btn btn-outline-dark mx-3">Сгенерировать ссылку для доступа
                                    к книге
                                </button>
                            </form>
                        @endif
            ...
            <div class="card-body">
                <blockquote class="blockquote mb-0">
                    <p>{{$book->text}}</p>
                    <footer class="blockquote-footer">Автор: <cite title="Source Title">{{$book->author->name}}</cite>
                    </footer>
                </blockquote>
            </div>
        </div>
